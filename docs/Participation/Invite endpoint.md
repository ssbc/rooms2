...
1. Once a web visitor opens that link, then they are presented with the SSB URI `ssb:invite/room/${msHost}/${msPort}/${inviteCode}/feed/ed25519/${roomShs}` where `${msHost}:${msPort}~shs:${roomShs}` consititute the room's multiserver address
